<script lang="ts">
	...
	$: {
		if (searchQuery.trim()) {
			// Client-side search through loaded blogs
			...
			filteredBlogs = blogs.filter(blog => blog.tags && blog.tags.includes(selectedTag));
		} else {
			filteredBlogs = blogs;
		}
	}
...
<main class="container mx-auto px-4 py-8" data-testid="blog-section">
	...
	{#if !searchQuery && !selectedTag && featuredBlogs.length > 0}
		<section class="mb-12">
			...
			>
				{#each featuredBlogs as blog (blog.id)}
					<BlogPreview {blog} />
				{/each}
			</div>
		</section>
