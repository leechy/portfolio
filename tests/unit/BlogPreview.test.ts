...
const mockBlogPostMinimal = {
	...
	readTimeMinutes: 5,
	featured: false
	// No coverImageUrl - testing without cover image
};
...
describe('BlogPreview Component', () => {
	describe('Rendering', () => {
		it('should render blog post preview with all required elements', () => {
			...
			expect(screen.getByTestId('blog-post-author')).toHaveTextContent(mockBlogPost.author);
		...
		it('should render cover image when provided', () => {
			...
			expect(coverImage).toHaveAttribute('src', mockBlogPost.coverImageUrl);
		...
		it('should display featured badge for featured posts', () => {
			...
			expect(screen.getByTestId('blog-featured-badge')).toHaveTextContent(/featured/i);
		});
