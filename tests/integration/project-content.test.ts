...
import { loadProjects, projectsStore } from '../../src/lib/stores/projects';
...
describe('T032 - Project Content Integration', () => {
  ...
  it('should provide project metadata for detailed pages', async () => {
    ...
    expect(project).toHaveProperty('demo_url');
  ...
  it('should provide project statistics for overview pages', async () => {
    ...
    const developmentProjects = state.projects.filter(p => p.status === 'in-progress');
